...
const InvoiceDetail = () => {
  ...
  if (!order) return <p>No invoice data</p> ;
      ...
      <div ref={pdfRef} className="max-w-4xl mx-auto p-8 rounded shadow" style={{ backgroundColor: "#ffffff", color: "#000000" }}>
        ...
        <div className="flex flex-col sm:flex-row justify-between border-b pb-4">
          ...
          <div className="sm:text-right text-xs sm:text-sm">
            <p><b>Invoice No:</b> {order.invoiceNumber}</p>
        ...
        {/* CUSTOMER + SHIPPING */}
        <div className="grid grid-cols-2 gap-3 sm:gap-6 mt-6 border-b pb-4 text-sm">
          <div>
        ...
        <div className="mt-6">
          <table className="w-full border text-sm" style={{ border: "1px solid #d1d5db" }}>
            <thead style={{ backgroundColor: "#e5e7eb" }}>
              <tr>
                <th style={{ border: "1px solid #d1d5db", padding: "8px" }}>Sr No</th>
                <th style={{ border: "1px solid #d1d5db", padding: "8px", textAlign: "left" }}>Item Name</th>
            ...
            <tbody>
              {(order.purchasedItems || []).map((item, i) => (
                <tr key={i}>
                  <td style={{ border: "1px solid #d1d5db", padding: "8px", textAlign: "center" }}>{i + 1}</td>
                  <td style={{ border: "1px solid #d1d5db", padding: "8px" }}>{item.bookName}</td>
                  <td style={{ border: "1px solid #d1d5db", padding: "8px", textAlign: "center" }}>{item.quantity}</td>
                  <td style={{ border: "1px solid #d1d5db", padding: "8px", textAlign: "center" }}>₹{item.price}</td>
